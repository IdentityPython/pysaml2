...
from saml2_tophat.s_utils import do_ava
from saml2_tophat import saml, ExtensionElement, NAMESPACE
from saml2_tophat import extension_elements_to_elements
from saml2_tophat import SAMLError
from saml2_tophat.saml import NAME_FORMAT_UNSPECIFIED, NAMEID_FORMAT_PERSISTENT, NameID
...
def ac_factory(path=""):
    ...
    if path:
        ...
        from saml2_tophat import attributemaps

        for typ in attributemaps.__all__:
            mod = import_module(".%s" % typ, "saml2_tophat.attributemaps")
            for key, item in mod.__dict__.items():
                if key.startswith("__"):
                    continue
                if isinstance(item, dict) and "to" in item and "fro" in item:
