...
import saml2_tophat
from saml2_tophat.s_utils import deflate_and_base64_encode
from saml2_tophat.sigver import REQ_ORDER, RESP_ORDER
from saml2_tophat.xmldsig import SIG_ALLOWED_ALG
...
def parse_soap_enveloped_saml(text, body_class, header_class=None):
    ...
    for part in envelope:
        ...
        if part.tag == '{%s}Body' % NAMESPACE:
            for sub in part:
                try:
                    body = saml2_tophat.create_class_from_element_tree(
                            body_class, sub)
                except Exception:
                    raise Exception(
            ...
            for sub in part:
                ...
                for klass in header_class:
                    ...
                    if sub.tag == "{%s}%s" % (klass.c_namespace, klass.c_tag):
                        header[sub.tag] = \
                            saml2_tophat.create_class_from_element_tree(klass, sub)
                        break
...
PACKING = {
    saml2_tophat.BINDING_HTTP_REDIRECT: http_redirect_message,
    saml2_tophat.BINDING_HTTP_POST: http_form_post_message,
}
